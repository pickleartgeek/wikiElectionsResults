...
const partyColors = { wc:"#2171b5", qtw:"#b71212", rf:"#b71212", gsx:"#691391", dvwb:"#F0B27A" };
const partyNames = { wc:"Worth-Cupcake", qtw:"TyroniusTheIII", rf:"Representative-Fee65", gsx: "gunsmokexeon", dvwb: "Disguised_VW_Beetle"};
...
// TABLE
function updateTable(candidates, name, forceNoProjection = false) {
...
if (!forceNoProjection) {
  ...
  // 2. Fallback 50% rule
  else if (sorted[0][1] / totalRaw > 0.5) {
    calledParty = winner;
  }
}
...
function showStatewide() {
  ...
  if (metaCall) {
    ...
    // 🚫 force NO projection
    updateTable(totals, "Statewide", true);
  }
}
